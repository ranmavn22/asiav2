    ...
    <?php if (is_tax('category_hotel')) {
        ...
        if ($obj) { ?>
            <aside id="category-<?php echo $id ?>" class="widget">
                ...
                <ul class="category menu">
                    <li><a href="<?php echo get_term_link($id) ?>"
                           title="<?php echo $parent ?>"><?php echo $parent ?></a>
                        <ul class="sub-menu">
                            <?php
                            if ($childs)
                                foreach ($childs as $child) {
...
if (is_singular('hotels')) {
    $obj = get_the_terms(get_the_ID(), 'category_hotel');
    $id = $obj[0]->term_id;
    $args = array('orderby' => 'name', 'parent' => $id, 'taxonomy' => 'category_hotel',);
    $childs = get_terms($args);
    $parent = $obj[0]->name;
    if (empty($childs)) {
        if ($obj[0]->parent != 0) {
            $id = $obj[0]->parent;
            $args2 = array('orderby' => 'id', 'parent' => $id, 'taxonomy' => 'category_hotel');
            $parent = get_the_category_by_ID($obj[0]->parent);
            $childs = get_terms($args2);
        }
    }
    if ($obj) { ?>
        <aside class="widget">
            ...
            <ul class="category menu">
                <li><a href="<?php echo get_term_link($id) ?>" title="<?php echo $parent ?>"><?php echo $parent ?></a>
                    <ul class="sub-menu">
                        ...
                        if ($childs)
                            foreach ($childs as $child) {
                                $args3 = array('orderby' => 'id', 'parent' => $child->term_id, 'taxonomy' => 'category_hotel');
                                $childs3 = get_terms($args3);
                                ?>
                                <li><a href="<?php echo get_term_link($child->term_id) ?>"
                                       title="<?php echo $child->name ?>"><?php echo $child->name ?></a>
                                    <ul class="sub-menu">
                                        ...
                                        if ($childs3)
                                            foreach ($childs3 as $child3) { ?>
                                                <li><a href="<?php echo get_term_link($child3->term_id) ?>" title="<?php echo $child3->name ?>"><?php echo $child3->name ?></a></li>
                                            <?php } ?>
                                    </ul>
                                </li>
                            <?php } ?>
